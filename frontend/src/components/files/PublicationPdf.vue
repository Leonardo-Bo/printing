<template>
    <div class="mt-4">
        <h5 class="inline-h mb-4">Publication PDF</h5>
        <div class="container-upload">
            <form @submit.prevent="uploadPublicationPDF">
    ...
    <div class="mt-4 container-upload">
        <table class="table table-striped table-hover table-sm">
            ...
            <tbody style="font-size: 13px;">
                <tr v-for="file in props.publication.pdf_publication" :key="file.id">
                    ...
                    <td>
                        ...
                        <abbr style="cursor: pointer;" title="Set as PDF">
                            <i 
                                @click="setPDF(file)" 
...
const uploadPublicationPDF = async () => {
    ...
    if (inputFiles.value.files.length > 0) {
        ...
        if(fnu.split('.').pop().toLowerCase() != "pdf") {
            ...
            formData.append('reffield', props.publication.id)
            formData.append('file', inputFiles.value.files[0])

            if (props.publication.pdf_publication.length == 0) {
                formData.append('isPDF', true)
            }

            await axios 
                .post('/api/v1/publication-files/', formData, { 
                    headers: {
...
const setPDF = async (file) => {
    ...
    for (let i = 0; i < props.publication.pdf_publication.length; i ++) {

        const fileID = props.publication.pdf_publication[i].id

        await axios
            .patch(`/api/v1/publication-files/${fileID}/`, formFalse, { 
                headers: {
                    "Content-Type": "multipart/form-data",
                }
            })
    ...
    await axios
        .patch(`/api/v1/publication-files/${file.id}/`, formTrue, { 
            headers: {
                "Content-Type": "multipart/form-data",
            }
        })
...
const deletePDF = async (file) => {
    await axios 
        .delete(`/api/v1/publication-files/${file.id}`)
        .then(response => {
            console.log(response)
            props.callback()
            createToast(
