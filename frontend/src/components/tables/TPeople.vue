<template>
    <div class="mb-4">
        <div class="row justify-content-between">
            <div class="col-4">
                ...
                </select> <span class="ms-2">research people per page</span>
            </div>
            <div class="col-4">
    ...
    <table class="table table-bordered">
        <thead>
            <tr>
                <th style="width: 34%;"><i class="bi bi-arrow-down-up float-end" @click="sort('name')"></i>Name</th>
        ...
        <tbody>
            <tr v-for="person in sortedPeople" v-bind:key="person.slug">
                <td>{{ person.name }}</td>
                <td>{{ person.position }}</td>
                <td>{{ person.author[0].username }}</td>
                <td>{{ formatDate(person.created_at) }}</td>
                <td>{{ person.author_update[0].username }}</td>
                <td>{{ formatDate(person.updated_at) }}</td>
                <td>
                    <abbr style="cursor: pointer;" title="Delete">
                        <i 
                            @click="$emit('personSlugDelete', person)" 
                            class="bi bi-trash text-danger float-end">
                        </i>
                    </abbr>
                    <abbr style="cursor: pointer;" title="Edit">
                        <i 
                            @click="$emit('personSlugEdit', person.slug)" 
    ...
    <div class="row py-3">
        <div class="col-7">
            <template v-if="filterCount != 1">
                {{ filterCount }} people found
            </template>
            <template v-else>
                {{ filterCount }} person found
            </template>
        </div>
...
defineEmits(['personSlugEdit', 'personSlugDelete'])

let copyPeople = props.people.map(row => { return row })
...
const hasFilter = (_people_, fil) => {
    if(fil.value !== '') {
        _people_ = _people_.filter(
            person => person.name.toLowerCase().indexOf(fil.value.toLowerCase()) >= 0 || 
            person.position.toString().indexOf(fil.value.toLowerCase()) >= 0 ||
            person.content.toLowerCase().indexOf(fil.value.toLowerCase()) >= 0 ||
            person.email.toLowerCase().indexOf(fil.value.toLowerCase()) >= 0 ||
            person.role.toLowerCase().indexOf(fil.value.toLowerCase()) >= 0
        );
    }
    return _people_
}
...
const sortedPeople = computed(() => {
    let _people = copyPeople;

    _people = hasFilter(_people, filter)
...
const pageCount = computed(() => {
    ...
    _people = hasFilter(_people, filter)
    return Math.ceil(_people.length / pageSize.value)
})
