<template>
    <div>
        <h4 class="inline-h mb-4">Add Publication</h4>
        ...
        <form @submit.prevent="createPublication">
            ...
            <div class="float-md-end mt-4 mb-4">
                <router-link type="button" class="btn btn-sm btn-outline-secondary ms-1" to="/settings/publications">Cancel</router-link>
                <button 
                    type="submit" 
...
const createPublication = async () => {
    ...
    if (!errors.value.length) {
        ...
        await axios
            .post('/api/v1/publications/', publication)
            .then(response => {
                // console.log(response)
                router.push(`/settings/publications/edit/${response.data.id}`)
                createToast(
